...
function wp_bootstrap_pagination( $args = array() ) {
    ...
    $defaults = array(
        'range'           => 4,
        'custom_query'    => FALSE,
        'previous_string' => __( '<i class="fa fa-caret-left pr-3"></i>', 'text-domain' ),
        'next_string'     => __( '<i class="fa fa-caret-right pl-2"></i>', 'text-domain' ),
    ...
    if ( !empty($min) && !empty($max) ) {
        for( $i = $min; $i <= $max; $i++ ) {
            if ($page == $i) {
                $echo .= '<li class="active page-item"><span class="active page-link">' . str_pad( (int)$i, STR_PAD_LEFT ) . '</span></li>';
            } else {
